...
//whent he mouse is moved on the screen this function is called with the move event as a parameter
...
function checkWin(){
	...
	//if there is no winner yet it will check if there are fouor in a row
	if(!win){
...
//this function takes the four chips in and checks to see if they are all the same color and they all exsist
function checkChipsWin(chipA, chipB, chipC, chipD){
	
	//it checks if they are all exsisting chips
	if(chipA.chip && chipB.chip && chipC.chip && chipD.chip){
		...
		if(chipB.color == chipAColor && chipC.color == chipAColor && chipD.color == chipAColor){
			...
			//if they are all the same color and exsisting it will change win to true
			win = true;
...
onkeydown = onkeyup = function(e){
	...
	if(map[37] && board.hand > 0){
		...
		//subtracting a value from the board hand is equivilent to moving it left
		board.hand--;
	...
	//when the right arrow is pressed and the hand is less than 6 it will moove the hand right
...
function Board(){
	...
	//when the board is created it will set up a 2d array for the chips and will give them a defualt value
	...
	this.changeSize = function(){
		...
		//this will check which dimention of the board is smaller and will center the baord on the larger of the two values
		if(width > height){
			...
			//if it is a square no offset is nessary and the default value of zero will remain unchanged
			this.size = width;
		}
	}
	...
	this.draw = function(){
		...
		for(let xVal = 0; xVal < this.chips.length; xVal++){
			for(let yVal = 0; yVal < this.chips[0].length; yVal++){
				...
				//sets the line with to be defined by the size so when the window is made small it doesnt look funny
